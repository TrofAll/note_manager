from datetime import datetime # Импортируем модуль из библиотеки


current_datetime = datetime.now() # Получаем текущую дату

formatted_datetime = current_datetime.strftime("%d-%m-%Y") # Преобразовываем дату в нужный нам формат

current_date = datetime.strptime(formatted_datetime, "%d-%m-%Y") # Преобразовываем строку
...
while True: # Начало цикла, будет продолжаться, пока ПЗ не введет корректную дату, в случае ошибки, повторяется
    try: # Запускаем блок проверки ошибок
        issue_date = input("Введите дату дедлайна (в формате день-месяц-год, например 20-01-2025): " ) # Просим ввести дату
        ...
        print("Неверный формат даты! Пожалуйста, введи правильный формат даты, например 20-01-2025!")