current_status = "На выполнении" #Создаем текущий статус заметки

print(f"Текущий статус заметки: {current_status}") #Выводит на экран текущий статус

new_status = input("""Выберите новый статус заметки путем ввода цифрового значения:
1. Выполнено 
2. В процессе
3. Отложено
""") #Запрашивает ввода нового статуса заметки

while new_status not in ["1", "2", "3"]: #Запускает правильность ввода, пока пользователь не выберет 1-3, будет выходить цикличный ввод
